...
for winst_server in winst_servers:
        ret_ap = subprocess.check_output(["ssh", "-o PreferredAuthentications=publickey", "-p 50000", "user@" + winst_server, "cat /root/winst/ap_serv | awk -F';' '{print $2}'"])
        ap_servers = ret_ap.splitlines()
        for ap_server in ap_servers:
                ap_ip = ap_server
                with open(os.devnull, 'w') as devnull:
			try:
	                        ret = subprocess.check_output(["ssh", "-o PreferredAuthentications=publickey", "-p 50000", "user@" + winst_server, "sshpass -p 'password' ssh -o ConnectTimeout=10 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no vendor_login@" + ap_ip + " \"wstalist; mca-status\""], stderr=devnull)
			except:
				e = exc_info()[0]
				data = {}
				data["ap_ip"] = ap_ip
